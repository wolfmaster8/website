...
function getMainInformation(info) {
    return {
        location: {city: info.name},
        atmospheric: {
...
async function getWeather(city = 'Bogota') {
    const weather = await $.ajax({
        url: `https://api.openweathermap.org/data/2.5/weather?APPID=9f1cb634ec747082eb0732356b6d147b&q=${city}`
    });
